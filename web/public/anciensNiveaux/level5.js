...
function create() {
    ...
    // Création du robot
    robot = this.physics.add.image(50, 350, "robot");

    // Création des astéroides
    asteroidesHaut = this.physics.add.group({
        key: "asteroide",
        repeat: 1,
        //set randomly the position of the asteroids
        setXY: {x: 300, y: 600},
    });

    for (let i = 0; i < 30; i++) {
        asteroidesHaut.create(Phaser.Math.Between(0, 1500), Phaser.Math.Between(0, 1500), "asteroide");
    }
...
function update() {
    ...
    for (let i = 0; i < this.sensorGraphics.length; i++) {
        ...
        asteroidesHaut.children.iterate(function (asteroide) {
            let data = determineFuturePoint(robot, asteroide, i, 1, null)
            if (data != null) moveTo(data, i)
        });
...
function determineFuturePoint(robot, asteroide, capteur, index, infos) {
    if (index > 3) {
        return null;
    } else {
        ...
        let futureRobot = {x: result.get([0]), y: result.get([1]), width: robot.width};
        let Iangle = Phaser.Math.DegToRad(startAngle + 30 * capteur);
        let ray = new Phaser.Geom.Line(
            futureRobot.x,
            futureRobot.y,
            futureRobot.x + Math.cos(Iangle) * 100 * index,
            futureRobot.y + Math.sin(Iangle) * 100 * index
        );
        if (Phaser.Geom.Intersects.LineToRectangle(ray, asteroide.getBounds())) {
            return {index: index, infos: infos};
        } else {
            return determineFuturePoint(futureRobot, asteroide, capteur, index + 1, null);
        }
    }
}
...
function moveTo(data, i) {
    if (i === 0) {
        let Nangle = Phaser.Math.DegToRad(-45 / (data.index + 1));
